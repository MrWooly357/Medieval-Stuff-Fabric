...
public final class AlternativesSpawnEntry extends LeafSpawnEntry {

    public static final MapCodec<AlternativesSpawnEntry> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ...
                    .apply(instance, AlternativesSpawnEntry::new)
    );

    private final List<SpawnEntry> entries;

    private AlternativesSpawnEntry(
    ...
    @Override
    protected SpawnEntryType<? extends AlternativesSpawnEntry> getType() {
        return SpawnEntryTypes.ALTERNATIVES;
    }

    @Override
    protected List<Pair<Entity, SpawnReason>> createEntities(SpawnContext context) {
        for (SpawnEntry entry : entries) {
            List<Pair<Entity, SpawnReason>> entities = entry.generateEntities(context);
    ...
    public static final class Builder extends LeafSpawnEntry.Builder<Builder> {
        ...
        public AlternativesSpawnEntry build(SpawnSelector.Data data, SpawnPosFinder posFinder, SpawnReason reason) {
            return new AlternativesSpawnEntry(data, List.copyOf(conditions), reason, posFinder, ignoreVanillaRules, List.copyOf(rules), List.copyOf(functions), List.copyOf(entries));
        }
